...
public class StatisticalAnomalyDetector extends CallStackAnomalyDetector {
     ...
     *            the n value
     */
    public StatisticalAnomalyDetector(CallStackArrays arrays, ITmfStateSystemBuilder ssb, int nValue) {
        super(arrays, ssb);
        fNValue = nValue;
    }
    ...
    protected void detectAnomalies() throws ClassNotFoundException, IOException {
        ...
        while (fArrays.hasNextArray()) {
            ...
            long address = array.getAddress();
            int depth = array.getDepth();
            long timestamp = array.getTimestamp();
            long duration = array.getDuration();
